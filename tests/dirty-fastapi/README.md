...
A deliberately broken FastAPI project with **27 intentional violations** across 7 categories. Used to validate that `/check`, `/fix-check`, `/fix-review`, and `/review-architecture` detect and fix real issues.
...
### Test 2: Validate `/fix-check` Harness (Delta Workflow)

After `/check` reports violations, run the self-correcting harness:
...
### Test 3: Validate `/fix-review` Harness (Full Audit Workflow)

Run the complete release gate pipeline:

```
# Step 1: Full audit (5 parallel agents)
/review-architecture

# Step 2: Self-correcting fix harness
/fix-review
```

**Expected**: REVIEW.md is produced with findings, then `/fix-review` fixes them in 1-2 iterations.

**What to measure**:
- **Detection coverage**: how many of the 27 violations appear in REVIEW.md
- **Fix convergence**: iterations needed (target: ≤2)
...
### `/fix-check` Harness Test (2026-03-25)
...
### `/fix-review` Harness Test (2026-03-25)

| Iteration | Violations | Δ | Fix Units | Key Event |
|-----------|-----------|---|-----------|-----------|
| 0 | 20 grep patterns failing | — | — | Full review (5 agents) |
...
The `/fix-check` test (high-level instructions) needed 3 iterations. The `/fix-review` test (prescriptive code) needed 1. **The more concrete the HOW TO FIX, the fewer loops needed.** This validates the Anthropic harness insight: sprint contracts reduce iteration count.
