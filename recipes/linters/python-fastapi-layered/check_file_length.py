"""Pre-commit check: file/function/class size limits per CLAUDE.md hard rules.

Files > 200 lines, functions > 30 lines, classes > 150 lines are blocking.
"""

import ast
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))
from _base import Violation, iter_python_files, run_checker

FILE_LINE_LIMIT = 200
FUNCTION_LINE_LIMIT = 30
CLASS_LINE_LIMIT = 150


class SizeLimitChecker(ast.NodeVisitor):
    def __init__(self, file_path: Path) -> None:
        self.file_path = file_path
        self.violations: list[Violation] = []

    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        self._check_function(node)
        self.generic_visit(node)

    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef) -> None:
        self._check_function(node)
        self.generic_visit(node)

    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        if node.end_lineno is not None:
            span = node.end_lineno - node.lineno + 1
            if span > CLASS_LINE_LIMIT:
                self.violations.append(
                    Violation(
                        self.file_path,
                        node.lineno,
                        f"Class '{node.name}' is {span} lines (limit: {CLASS_LINE_LIMIT})",
                    )
                )
        self.generic_visit(node)

    def _check_function(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> None:
        if node.end_lineno is None:
            return
        span = node.end_lineno - node.lineno + 1
        if span > FUNCTION_LINE_LIMIT:
            self.violations.append(
                Violation(
                    self.file_path,
                    node.lineno,
                    f"Function '{node.name}' is {span} lines (limit: {FUNCTION_LINE_LIMIT})",
                )
            )


def check_file(file_path: Path, tree: ast.AST) -> list[Violation]:
    violations: list[Violation] = []
    try:
        line_count = len(file_path.read_text(encoding="utf-8").splitlines())
    except OSError:
        line_count = 0
    if line_count > FILE_LINE_LIMIT:
        violations.append(
            Violation(file_path, 1, f"File is {line_count} lines (limit: {FILE_LINE_LIMIT})")
        )
    checker = SizeLimitChecker(file_path)
    checker.visit(tree)
    violations.extend(checker.violations)
    return violations


def _is_excluded(file_path: Path) -> bool:
    if file_path.name == "__init__.py":
        return True
    parts = file_path.parts
    return "alembic" in parts and "versions" in parts


def main() -> int:
    src_dir = Path("src")
    if not src_dir.exists():
        print("src/ directory not found")
        return 1
    files = [f for f in iter_python_files(src_dir) if not _is_excluded(f)]
    return run_checker(check_file, files, "Module size limits (file/function/class)")


if __name__ == "__main__":
    sys.exit(main())
