...
CLAUDE.md hard rule: routers are HTTP-only. DB concerns (sessions, queries,
relationship loading) live in repositories. import-linter enforces this for
`shipboard.repositories`/`shipboard.models`; this check covers the runtime
package `sqlalchemy` itself.
"""
            ...
            if alias.name == "sqlalchemy" or alias.name.startswith("sqlalchemy."):
                self.violations.append(
                    Violation(
                        self.file_path,
                        node.lineno,
                        f"Router imports `{alias.name}` — DB concerns belong in a repository",
                    )
                )
        ...
        if module == "sqlalchemy" or module.startswith("sqlalchemy."):
            self.violations.append(
                Violation(
                    self.file_path,
                    node.lineno,
                    f"Router imports from `{module}` — DB concerns belong in a repository",
                )
            )
...
def main() -> int:
    routers_dir = Path("src/shipboard/routers")
    if not routers_dir.exists():
        print("src/shipboard/routers/ does not exist yet — skipping")
        return 0
